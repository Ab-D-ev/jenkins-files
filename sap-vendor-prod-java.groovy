pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                git branch: 'main', credentialsId: 'Omkar', url: 'git@github.com:OmkarMore-1999/SAPVendorPortal.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Deploy') {
            steps {
               sh 'scp -o StrictHostKeyChecking=no -i ${SSH_KEY_PATH} /var/lib/jenkins/workspace/sap-vendor-java-test/target/VendorPortal.war java3@20.244.110.102:/home/java3/sap-prod.war'
                sshagent(['Java3']) {
                    sh 'ssh -o StrictHostKeyChecking=no java3@20.244.110.102 "sudo mv /home/java3/sap-prod.war /opt/tomcat/webapps/sap-prod.war"'
                }
            }
        }
    }
    ...
    post {
        success {
            script {
                emailext body: '''<html><body><h2>SAP Vendor WAR Deployed - Version # $BUILD_NUMBER - Deployment $BUILD_STATUS.</h2><br/>
                        <br/>
                    ...
                    subject: 'SAP Vendor WAR Deployed Successfully - $PROJECT_NAME',
        ...
        failure {
            script {
                emailext body: '''<html><body><h2>SAP Vendor WAR Failed - Build # $BUILD_NUMBER - Deployment $BUILD_STATUS.</h2><br/>
                        <br/>
